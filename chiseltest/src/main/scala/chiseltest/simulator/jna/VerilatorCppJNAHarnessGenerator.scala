...
struct sim_state {
  ...
  inline int64_t step(int32_t cycles) { //here
  ...
  inline uint32_t* getCovVector() { return dut->getCovVector(); }
  inline uint32_t covVectorSize() { return dut->covVectorSize(); }
