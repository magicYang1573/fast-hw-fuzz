...
for i in range(args.iterations):
    ...
    print("Calling FastDriver with arguments: {SCALA_ARGS}\n".format(
        SCALA_ARGS=scala_args))

    os.system("java -cp target/scala-2.12/rtl-fuzz-lab-assembly-0.1.jar fuzzing.fast.FastDriver {SCALA_ARGS} --OutputFolder {FOLDER} &".format(
    ...
    # return_code = subprocess.call(["java", "-cp target/scala-2.12/rtl-fuzz-lab-assembly-0.1.jar fuzzing.fast.FastDriver {SCALA_ARGS} --OutputFolder {FOLDER} &".format(
    #     SCALA_ARGS=scala_args, FOLDER=out_folder_run)])
    # print(return_code)
